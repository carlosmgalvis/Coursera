...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		// We iterate through every possible destination node 'k'
		for (int k = 0; k < n; k++) {
			int pathsOfLength2 = 0;
			
			// Compute the dot product of row v and column k
			// This represents the number of paths: v -> j -> k
			for (int j = 0; j < n; j++) {
				pathsOfLength2 += (adjMatrix[v][j] * adjMatrix[j][k]);
			}
			
			// Add the vertex 'k' to the list as many times as there are paths
			for (int i = 0; i < pathsOfLength2; i++) {
				distance2Nodes.add(k);
			}
		}
