...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            // 1. Check if the token is a word
            if (isWord(token)) {
                numWords++;
                // Count syllables using the efficient helper (no regex inside)
                numSyllables += countSyllables(token);
            }

            // 2. Check if the token is a sentence ender
            // Criteria: matches punctuation OR it's the last token in the document
            if (token.matches("[.!?;]+") || (i == tokens.size() - 1 && numWords > 0 && !token.trim().isEmpty())) {
                // If it's the last token and we haven't counted this sentence yet 
                // (and it's not just punctuation we already counted), add a sentence.
                // However, the standard logic is: if it's punctuation, it's a sentence.
                if (token.matches("[.!?;]+")) {
                    numSentences++;
                }
            }
        ...
        // Final check: If document ends without punctuation but contains words, count that last sentence
        if (tokens.size() > 0) {
            String lastToken = tokens.get(tokens.size() - 1);
            if (!lastToken.matches("[.!?;]+") && numWords > 0) {
                numSentences++;
            }
        }
    }
