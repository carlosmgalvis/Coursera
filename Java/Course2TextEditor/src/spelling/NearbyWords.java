...
//		List<String> retList = new LinkedList<String>();   // words to return
		...
		while (!queue.isEmpty() && retList.size() < numSuggestions && processedCount < THRESHOLD) {
            String curr = queue.poll();
            processedCount++;

            // Obtenemos todas las variantes a distancia 1 (aunque no sean palabras reales)
            ...
            for (String n : neighbors) {
                if (!visited.contains(n)) {
                    ...
                    // Si es una palabra válida del diccionario, la añadimos a sugerencias
                    if (dict.isWord(n)) {
                        retList.add(n);
