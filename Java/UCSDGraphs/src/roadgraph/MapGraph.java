...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		//nodeSearched.accept(next.getLocation());
		return searchAlgorithm(start, goal, nodeSearched, "Dijkstra");
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		return searchAlgorithm(start, goal, nodeSearched, "AStar");
	}
	
	/**
	 * Generic search algorithm to handle both Dijkstra and A*
	 * They share the same logic, differing only in the priority calculation.
	 */
	private List<GeographicPoint> searchAlgorithm(GeographicPoint start, GeographicPoint goal, 
	                                              Consumer<GeographicPoint> nodeSearched, String type) {
	    // Basic validation
	    ...
	    for (MapNode n : pointNodeMap.values()) {
	        ...
	        n.setActualDistance(Double.POSITIVE_INFINITY); // Path cost only
	    }
	    ...
	    while (!pq.isEmpty()) {
	        ...
	        if (!visited.contains(curr)) {
	            ...
	            for (MapEdge edge : curr.getEdges()) {
	                ...
	                if (!visited.contains(neighbor)) {
	                    // Path cost to neighbor
	                    double currentDist = curr.getActualDistance() + edge.getLength();
	                    
	                    // A* adds the straight-line distance to the goal as a heuristic
	                    double priorityDist = currentDist;
	                    if (type.equals("AStar")) {
	                        priorityDist += neighbor.getLocation().distance(goal);
	                    }

	                    if (currentDist < neighbor.getActualDistance()) {
	                        neighbor.setActualDistance(currentDist);
	                        neighbor.setDistance(priorityDist);
