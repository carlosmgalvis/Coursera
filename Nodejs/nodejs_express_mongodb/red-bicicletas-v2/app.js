...
app.set('secreteKey', 'jwt_pwd_!!223344');
...
app.post('/forgotPassword', function(req,res, next){
  Usuario.findOne({ email: req.body.email }, (err, user) => {
    if (!user)
      return res.render("session/forgotPassword", { info: { message: "Mail no verificado." }});
...
app.use('/googlec859f2728abf868e', function(req,res){
  res.sendFile('public/googlec859f2728abf868e');
});

app.get('/auth/google', passport.authenticate('google',{ scope: [ 'https://www.googleapis.com/auth/plus.login','https://www.googleapis.com/auth/plus.progile.emails.read']}));
...
function validarUsuario(req,res,next){
  jwt.verify(req.headers('x-access-token'), req.app.get('secretKey'), function(err,decoded){
    ...
    else{ req.body.userId = decoded.id; console.log('jwt verify: '+ decoded); next(); }
  });
}
